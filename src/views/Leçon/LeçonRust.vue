...
const LS_KEY = 'codequest_rust_progress'
const LANGAGE = 'rust'
...
const curriculum: CourseModule[] = CURRICULUM_RUST
...
function runChallenge(si: number, step: LessonStep) {
  ...
  let output = '$ cargo run\n'
  const prints = [...code.matchAll(/println!\s*\(([^)]+)\)/g)]
...
function resetAllProgress() {
  if (!confirm('Réinitialiser toute ta progression Rust ?')) return
  localStorage.removeItem(LS_KEY)
  location.reload()
}
