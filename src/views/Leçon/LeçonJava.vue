...
const LS_KEY = 'codequest_java_progress'
const LANGAGE = 'java'
...
const curriculum: CourseModule[] = CURRICULUM_JAVA
...
function runChallenge(si: number, step: LessonStep) {
  ...
  let output = '$ javac Main.java && java Main\n'
  const prints = [...code.matchAll(/System\.out\.println\s*\(([^)]+)\)/g)]
...
function resetAllProgress() {
  if (!confirm('Réinitialiser toute ta progression Java ?')) return
  localStorage.removeItem(LS_KEY)
  location.reload()
}
