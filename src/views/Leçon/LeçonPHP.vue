...
const curriculum: CourseModule[] = CURRICULUM_PHP
...
function runChallenge(si: number, step: LessonStep) {
  ...
  let output = '$ php main.php\n'
  const prints = [...code.matchAll(/echo\s+([^\n;]+)/g)]
...
function resetAllProgress() {
  if (!confirm('Réinitialiser toute ta progression PHP ?')) return
  localStorage.removeItem(LS_KEY)
  location.reload()
}
