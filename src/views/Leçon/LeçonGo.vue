...
const LS_KEY = 'codequest_go_progress'
const LANGAGE = 'go'
...
const curriculum: CourseModule[] = CURRICULUM_GO
...
function runChallenge(si: number, step: LessonStep) {
  ...
  let output = '$ go run main.go\n'
  const prints = [...code.matchAll(/fmt\.Println\s*\(([^)]+)\)/g)]
...
function resetAllProgress() {
  if (!confirm('Réinitialiser toute ta progression Go ?')) return
  localStorage.removeItem(LS_KEY)
  location.reload()
}
