...
async function scrapeSource(source: typeof RSS_SOURCES[0]): Promise<number> {
    try {
        ...
        for (const item of feed.item) {
            ...
            try {
                ...
                if (err.code !== 11000) {
                    console.error(`Erreur sauvegarde : ${item.title}`, err.message)
                }
            }
        }

        console.log(`${source.name} : ${savedCount} articles traités`)
        return savedCount
...
export async function scrapeAllSources(): Promise<void> {
    ...
    const results = await Promise.allSettled(
        RSS_SOURCES.map(source => scrapeSource(source))
    )
